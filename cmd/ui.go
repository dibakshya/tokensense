...
func PrintSetupComplete(proxyStarted bool) {
	...
	warn := bold(yellow("  ⚠️   RELAUNCH YOUR AI TOOLS — or nothing will be tracked yet"))
	sub  := "      " + yellow("Quit and reopen Cursor, Claude Desktop, VS Code, Windsurf — any AI app you use.")
	...
	if proxyStarted {
		...
		fmt.Println()
		fmt.Println(green(w + "  ✅  Step 2 — system proxy configured automatically"))
		fmt.Println(w + "           All apps will route through Tokensense without")
		fmt.Println(w + "           needing a terminal restart.")
		fmt.Println()
		fmt.Println(bold(w+"  Step 3 →  ") + bold(yellow("Quit and relaunch your AI apps")))
		fmt.Println(w + "           Cursor, Claude Desktop, VS Code, Windsurf — any app")
		fmt.Println(w + "           that calls AI APIs needs to restart once to pick up")
		fmt.Println(w + "           the new proxy settings.")
		fmt.Println(w + "           ↳ " + yellow("Nothing will be tracked until you do this."))
		fmt.Println()
		fmt.Println(green(w + "  ✅  Step 4 — browser dashboard is opening now…"))
		...
		fmt.Println()
		fmt.Println(bold(w+"  Step 2 →  ") + bold(yellow("Quit and relaunch your AI apps")))
		fmt.Println(w + "           Cursor, Claude Desktop, VS Code, Windsurf — any app")
		fmt.Println(w + "           that calls AI APIs needs to restart once to pick up")
		fmt.Println(w + "           the new proxy settings.")
		fmt.Println(w + "           ↳ " + yellow("Nothing will be tracked until you do this."))
