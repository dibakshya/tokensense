...
func PrintSetupComplete(proxyStarted bool) {
	...
	warn := bold(yellow("  ⚠️   RESTART TERMINAL + RELAUNCH AI TOOLS — or nothing will be tracked"))
	sub  := "      " + yellow("Open a new terminal tab, then quit+reopen Cursor, Claude Desktop, VS Code, etc.")
	...
	if proxyStarted {
		...
		fmt.Println()
		fmt.Println(bold(w+"  Step 2 →  ") + bold(yellow("Restart your terminal + relaunch your AI tools")))
		fmt.Println(w + "           Open a NEW terminal tab, then quit and relaunch")
		fmt.Println(w + "           Cursor, Claude Desktop, VS Code, Windsurf — any app")
		fmt.Println(w + "           that calls AI APIs.")
